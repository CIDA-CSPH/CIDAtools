...
4. [Quitting RStudio/JupyterLab Sessions](#quit-rstudiojupyterlab-session) - How to quit an RStudio/JupterLab session.
5. [Quickstart](#quickstart) - Quick review of all topics.
...
### A1. Motivation
Using RStudio/JupyterLab on the cluster allows you to:

- Utilize the cluster's [computational resources](https://cida-csph.github.io/CIDAtools/articles/CIDA_BIOS_Cluster.html#csph-biostats-hpc-cluster) while coding interactively in a familiar environment.
- Running RMarkdown/Quarto/Jupyter Notebooks directly on the cluster.
- Generate and view visualizations in real-time.

### A2. Capabilities

The RStudio inteface runs R 4.5.0, and supports the usual RStudio functions including RMarkdown/Quartio notebooks, interactive plotting, etc. For convenience, the RStudio container also has a variety of commonly used packages pre-installed, including the `tidyverse` family.

The JupyterLab interface supports running Jupyter Notebooks (`.ipynb`), and supports all JupyterLab functions. By default, the JupyterLab container has the following kernels installed:
...
Although the RStudio/JupyterLab instance will run on an HPC compute node, you'll log in to the cluster normally (via SSH), and access the RStudio interface through your local browser.

In order for this to work correctly, we need to configure a few SSH options when logging in to the cluster.

These SSH options should only need to be configured once on your local computer (but if you switch to a different computer, you'll need to re-do these SSH configuration steps on that computer as well).

### 1A. Locate SSH Config File

Although you can pass parameters when calling `ssh` on the command line, the simplest way to configure SSH options is to use the SSH config file.
...
**Note:** Some GUI clients (like PuTTY on Windows) do not support port forwarding to Unix sockets (requried functionality to use RStudio/JupyterLab on the cluster). However, command-line versions of SSH on both Mac and Windows do support this.

The SSH config file is useful because it allows us to define persistent options which will be used each time we log in to the cluster (rather than having to remember which parameters to pass on the command line).
...
If you see these error messages in your SSH window, Congratulations! This means you've configured your SSH connection correctly. The error messages appear because the browser is attempting to connect, but there is nothing on the other end (yet) to accept the connection.
...
To launch an RStudio/JupyterLab session, you'll need a copy of the `rstudio_helper.sh` and `jupyterlab_helper.sh` scripts. These scripts help automate some of the setup for RStudio/JupyterLab sessions.
...
The sections below provide some additional configuration options and information, but these sections are 
