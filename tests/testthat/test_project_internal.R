...
test_that("test the get_default_path function",{
  ...
  dcf_content <- list( path="~/test_drive2", analyst_name="Analyst Name")
  write.dcf(dcf_content, file.path(global_path))
  w <- capture_warnings(test_path <- get_default_path())
  full_path_test <- as.character(fs::path_abs("~/test_drive2"))
  expect_equal(test_path,full_path_test)

  #Create Project MetaData File
  setwd(paste(test_drive_dir,test_project_path,sep=""))
  dcf_content <- list( ProjectName="Test Project 5", PI="Dr. Test",analyst="Analyst Name",datalocation="Branches/dr_test/test_project",gitlocation="CIDATools/Test_test_project",default_full_path_to_project="~/test_drive3/Branches/dr_test/test_project")
  w <- capture_warnings(save_project_data(dcf_content))
  expect_match(w,".ProjData/Data.dcf File does not exist.",all=FALSE)


  w <- capture_warnings(test_path <- get_default_path())
  full_path_test <- fs::path_abs("~/test_drive3")
  expect_equal(test_path,full_path_test)
